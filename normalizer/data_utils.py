...
def add_audio_preprocessor_args(parser):
    ...
    parser.add_argument(
        ...
        type=int,
        default=16000,
        help="Output sample rate for --audio_preprocessor krisp_bvc.",
    )
    parser.add_argument(
        "--krisp_bvc_pad_seconds",
        type=float,
        default=2.0,
        help="Leading silence pad to add before Krisp BVC processing and trim after.",
    )
    parser.add_argument(
...
def _load_krisp_bvc_config(project_dir, sample_rate, pad_seconds):
    _load_dotenv("krisp_bvc")

    missing_env = [name for name in AI_COUSTICS_LIVEKIT_ENV if not os.environ.get(name)]
    if missing_env:
        raise RuntimeError(
            "--audio_preprocessor krisp_bvc requires "
            + ", ".join(missing_env)
            + " to be set in the eval environment."
        )

    if sample_rate <= 0:
        raise ValueError("--krisp_bvc_sample_rate must be greater than 0")
    if pad_seconds < 0:
        raise ValueError("--krisp_bvc_pad_seconds must be non-negative")
    if shutil.which("uv") is None:
        raise RuntimeError(
            "--audio_preprocessor krisp_bvc requires the 'uv' executable."
        )
...
def _process_audio_with_krisp_bvc(audio, config):
    import numpy as np
    import soundfile as sf
    ...
    with tempfile.TemporaryDirectory(prefix="asr-krisp-bvc-") as tmpdir:
        tmp_path = Path(tmpdir)
        padded_path = tmp_path / "input_padded.wav"
        output_path = tmp_path / "output.wav"
        ...
        if result.returncode != 0:
            stderr = (result.stderr or result.stdout or "").strip()
            raise RuntimeError(
                "Krisp BVC preprocessing failed with exit code "
                f"{result.returncode}: {stderr}"
            )
...
def _build_audio_preprocess_fn(
    ...
    krisp_bvc_project_dir=None,
    krisp_bvc_sample_rate=16000,
    krisp_bvc_pad_seconds=2.0,
):
    ...
    if audio_preprocessor == "krisp_bvc":
        config = _load_krisp_bvc_config(
            krisp_bvc_project_dir,
            krisp_bvc_sample_rate,
            krisp_bvc_pad_seconds,
        )
        ...
        def preprocess_audio(batch):
            batch["audio"] = [
                _process_audio_with_krisp_bvc(audio, config)
                for audio in batch["audio"]
            ]
            return batch
...
def prepare_data(dataset, sampling_rate=16000, args=None):
    ...
    krisp_bvc_project_dir = _get_arg(args, "krisp_bvc_project_dir", None)
    krisp_bvc_sample_rate = _get_arg(args, "krisp_bvc_sample_rate", 16000)
    krisp_bvc_pad_seconds = _get_arg(args, "krisp_bvc_pad_seconds", 2.0)
