...
def main(args):
    ...
    dataset = load_dataset(
        args.dataset, CONFIG_NAME, split=SPLIT_NAME, streaming=args.streaming
    )
    ...
    def download_audio_files(batch):
        ...
        for i, (file_name, sample, duration, text) in enumerate(
            zip(batch["file_name"], batch["audio"], batch["duration"], batch["text"])
        ):
            # Create unique filename using index to avoid conflicts
            unique_id = (
                f"{CONFIG_NAME}_{i}_{os.path.basename(file_name).replace('.wav', '')}"
            )
