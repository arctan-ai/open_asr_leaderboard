...
def parse_args():
    ...
    parser.add_argument(
        "--csv_file",
        default="scripts/data/multilingual.csv",
        help="Path to the multilingual leaderboard CSV file (default: scripts/data/multilingual.csv).",
    )
    parser.add_argument(
        "--label-col",
        default="model",
        help="Column name for model labels (default: model).",
    )
    parser.add_argument(
        "--rtfx-col",
        default="RTFx",
        help="Column name for RTFx values (default: RTFx).",
    )
    parser.add_argument(
        "--size-col",
        default="Model size (B)",
        help="Column name for model size (default: 'Model size (B)').",
    )
    parser.add_argument(
        "--exclude-cols",
        nargs="*",
        default=None,
        help="Columns to exclude when computing avg WER. Defaults to --label-col, --size-col, and --rtfx-col.",
    )
    ...
    parser.add_argument(
        ...
        default=[3, 8],
        metavar=("MIN", "MAX"),
        help="WER axis limits for both plots (default: 3 12).",
    )
    ...
    parser.add_argument(
        ...
        default=[1e1, 1e4],
        metavar=("MIN", "MAX"),
        help="Y-axis limits for the RTFx plot (default: 1e1 1e4).",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        metavar=("MIN", "MAX"),
        help="Y-axis limits for the model-size plot (default: None).",
    )
    parser.add_argument(
        "--size-yfact",
        type=float,
        default=1e3,
        help="Multiplicative factor for model size values, e.g. 1e3 to convert B to M (default: 1000).",
    )

    # Output files
    parser.add_argument(
        "--rtfx-output",
        default="multilingual_rtfx_wer.png",
        help="Output filename for the RTFx plot (default: multilingual_rtfx_wer.png).",
    )
    parser.add_argument(
        "--size-output",
        default="multilingual_size_wer.png",
        help="Output filename for the model-size plot (default: multilingual_size_wer.png).",
    )
    ...
    parser.add_argument(
        "--highlight",
        default=None,
        help="Model name to highlight with a star marker and red label (default: None).",
    )
    ...
    parser.add_argument(
        "--custom-model",
        default=None,
        help="Custom model to add to the plot in format 'model,avg_wer,rtfx,size' (e.g., 'MY MODEL,6.5,800,2.0').",
    )
...
if __name__ == "__main__":
    ...
    if args.highlight is not None:
        if args.highlight not in df[args.label_col].values:
            print(
                f"Error: Model '{args.highlight}' not found in CSV file '{args.csv_file}'"
            )
    ...
    exclude = (
        set(args.exclude_cols)
        if args.exclude_cols
        else {args.label_col, args.size_col, args.rtfx_col}
    )
    lang_columns = [
        col for col in df.columns if col not in exclude and col != "Avg. WER"
    ]
